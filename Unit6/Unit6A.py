...
class Unit6A:
    @staticmethod
    def unit6a():
        @Token.bot.message_handler(func=lambda message: message.text == '📚 Unit 6A' or message.text == '/Unit6A')
        def website(message):
            ...
            mess_1 = """
<b>Unit3 6A What the waiter really thinks</b>
"""
...
@Token.bot.message_handler(func=lambda message: message.text == '📝 Unit3 6A Test' or message.text == '/Unit6A_Test')
...
def check_answer5(inner_message, score):
    ...
    Token.bot.send_message(inner_message.chat.id,
                     text="Next /Unit6B\nOr:/Unit6A_Test to "
                          "try again")
    if answer == '/Unit6B':
        Token.bot.register_next_step_handler(inner_message, Unit6B)
