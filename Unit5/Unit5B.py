...
class Unit5B:
    @staticmethod
    def unit5b():
        @Token.bot.message_handler(func=lambda message: message.text == '📚 Unit 5B' or message.text == '/Unit5B')
        def website(message):
...
@Token.bot.message_handler(func=lambda message: message.text == '📝 Unit3 5B Test' or message.text == '/Unit5B_Test')
...
def check_answer5(inner_message, score):
    ...
    Token.bot.send_message(inner_message.chat.id,
                     text="Next:/Unit6A\nOr:/Unit5B_Test to "
                          "try again")
    if answer == '/Unit6A':
        Token.bot.register_next_step_handler(inner_message, Unit6A)
