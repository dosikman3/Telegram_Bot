...
class Unit1B:
    @staticmethod
    def unit1b():
        @Token.bot.message_handler(func=lambda message: message.text == '📚 Unit 1B' or message.text == '/Unit1B')
        def website(message):
...
"""
            ...
            Token.bot.send_message(message.chat.id,
                                   "Well done, let's pass the test /Unit_1B_Test or move on to the next section' \n/Unit2A")


@Token.bot.message_handler(func=lambda message: message.text == '📝 Unit3 1B Test' or message.text == '/Unit_1B_Test')
def test(inner_message):
    score = 0
    chat_id = inner_message.chat.id
    Token.bot.send_message(chat_id=chat_id,
                           text="1.Which color is traditionally associated with love and can be used in bedrooms to create a peaceful or passionate atmosphere?"
                                "\n1)Red."
                                "\n2)Orange."
                                "\n3)Pink.")
...
\nOr press:/Unit_1B_Test for replay
                     """)
    if answer == '/Unit2A':
        Token.bot.register_next_step_handler(inner_message, Unit2A)
