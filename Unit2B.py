...
class Unit2B:
    @staticmethod
    def unit2b():
        @bot.message_handler(func=lambda message: message.text == '📚 Unit 2B' or message.text == '/Unit2B')
        def website(message):
            markup = types.InlineKeyboardMarkup()

            mess_1 = """
<b>Unit 2B Go to checkout</b> 
"""
...
@bot.message_handler(func=lambda message: message.text == '📝 Unit 2B' or message.text == '/Unit2B_Test')
...
def check_answer5(inner_message, score):
    ...
    bot.send_message(inner_message.chat.id,
                     text="Next Unit:/Unit3A\nOr press:/Unit2B_Test to try again")
    if answer == '/Unit3A':
        bot.register_next_step_handler(inner_message, Unit3A)
